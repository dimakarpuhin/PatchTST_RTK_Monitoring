# two_stage_training_pm_like.py
# Двухэтапное обучение с синтетикой, похожей на PM
...
from synthetic_pm_like import SyntheticPMLikeGenerator
...
def generate_synthetic_pm_like(config, samples_per_class=500):
    """Генерация синтетики, похожей на PM"""
...
def adapt_classifier(model, new_num_classes):
    ...
    print(f"  ✅ Классификатор адаптирован: {old_classifier.out_features} → {new_num_classes} классов")
    return model

def main():
    print("=" * 70)
    print("🔬 ДВУХЭТАПНОЕ ОБУЧЕНИЕ (СИНТЕТИКА PM-LIKE → PM)")
    print("=" * 70)
    ...
    Config.NUM_CHANNELS = X_train.shape[2]  # 5
    ...
    # ===== 1. Обучение с нуля =====
    print("\n📚 ЭКСПЕРИМЕНТ 1: Обучение с нуля (базовый уровень)")
    ...
    # ===== 2. Предобучение на новой синтетике =====
    print("\n📚 ЭКСПЕРИМЕНТ 2: Синтетика PM-like → PM")
    
    X_synth, y_synth = generate_synthetic_pm_like(Config, samples_per_class=500)
    synth_loader, _, _ = create_dataloaders(
        X_synth, y_synth, X_synth[:10], y_synth[:10], X_synth[:10], y_synth[:10], Config.BATCH_SIZE
    )
    
    model = create_model(Config)
    print("\n📚 Предобучение на синтетике PM-like...")
    trainer = Trainer(model, Config)
    ...
    results.append({
        'experiment': 'Синтетика PM-like → PM',
        'val_acc': round(best_val_acc, 2),
        'test_acc': round(test_acc, 2),
        'test_f1': round(f1, 4),
        'time_sec': round(elapsed, 1)
    ...
    print("\n" + "=" * 70)
    print("📊 СВОДНАЯ ТАБЛИЦА (СИНТЕТИКА PM-LIKE)")
    print("=" * 70)
    print(df.to_string(index=False))
    
    os.makedirs('logs', exist_ok=True)
    df.to_csv('logs/two_stage_pm_like_results.csv', index=False)
    print("\n💾 Результаты сохранены в logs/two_stage_pm_like_results.csv")
