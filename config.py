...
class Config:
    ...
    DROPOUT = 0.2            # Вероятность dropout Эксперимент 2 - увеличиваем число случайных нейронов в при обучении сети 0.1 -10%, 0.2 - 20%
    
    # ========== Параметры регуляризации (глава 2, формула 2.8) ==========
    MASK_PROB = 0.15         # Вероятность стохастического маскирования патчей p_mask
    LAMBDA_1 = 5e-4          # Коэффициент L2-регуляризации λ₁ - Эксперимент 2 - увеличиваем регуляризацию L2 делая веса модели меньше и стабильнее
    ...
    SYNTHETIC_SAMPLES_PER_CLASS = 100   # количество образцов на класс для генерации, в v1.0 ставил 24 образца на класс, теперь изменил на 50, 200, 500, 1000, 2000, для повышения точности
